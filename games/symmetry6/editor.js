...
var firebaseConfig = {
  apiKey: "AIzaSyAFHowKHdoFzePmrTMIcGEHiDXtpQGsFgk",
  authDomain: "symmetry6-levels.firebaseapp.com",
  projectId: "symmetry6-levels",
  storageBucket: "symmetry6-levels.firebasestorage.app",
  messagingSenderId: "1081538747875",
  appId: "1:1081538747875:web:a2eddf348d3219698014d6",
  measurementId: "G-TZCMHM2CJG"
};
...
function setupEvents() {
  ...
  let machineId = localStorage.getItem("hexGridMachineId");
  if (!machineId) {
    machineId = "m_" + Math.random().toString(36).substr(2, 9) + "_" + Date.now();
    localStorage.setItem("hexGridMachineId", machineId);
  }
  let hostApiAvailable = true;
  let defaultAuthorName = localStorage.getItem("hexGridAuthorName") || "Player";
  let defaultLevelName = "MyLevel";
  let defaultHostFilename = `${defaultAuthorName}_${defaultLevelName}`;
  document.querySelectorAll(".host-btn").forEach((btn) => btn.style.display = "inline-block");
  document.getElementById("btn-host-save").addEventListener("click", async () => {
    if (!hostApiAvailable)
      return;
    disableTestMode();
    saveCurrentToData();
    const authorName = prompt("Enter your Author Name:", defaultAuthorName);
    if (!authorName)
      return;
    localStorage.setItem("hexGridAuthorName", authorName);
    defaultAuthorName = authorName;
    const levelName = prompt("Enter Level Name:", defaultLevelName);
    if (!levelName)
      return;
    defaultLevelName = levelName;
    const filename = `${authorName}_${levelName}`.replace(/[^a-zA-Z0-9_\-\.]/g, "_");
    try {
      await setDoc(doc(db, "custom_levels", filename), {
        machineId,
        authorName,
        levelName,
      ...
      alert(`Successfully saved to cloud as ${filename}`);
      defaultHostFilename = filename;
    } catch (err) {
      console.error("Firebase save error:", err);
      if (err.code === "permission-denied") {
        alert("Permission Denied: You cannot overwrite someone else's level!");
      } else {
        alert("Error saving to cloud: " + err.message);
      }
    }
  });
  document.getElementById("btn-host-load").addEventListener("click", async () => {
    if (!hostApiAvailable)
      return;
    try {
      const querySnapshot = await getDocs(collection(db, "custom_levels"));
      const select = document.getElementById("host-file-select");
      select.innerHTML = "";
      if (!querySnapshot.empty) {
        querySnapshot.forEach((doc2) => {
          ...
          option.value = file;
          option.text = file;
          if (file === defaultHostFilename)
            option.selected = true;
          option.dataset.levels = doc2.data().levelsJson;
          select.appendChild(option);
        });
      } else {
        const option = document.createElement("option");
        option.text = "No custom levels found in cloud";
        option.disabled = true;
        select.appendChild(option);
      }
      document.getElementById("host-load-modal").showModal();
    } catch (err) {
      alert("Error fetching files from cloud: " + err.message);
    }
  });
  document.getElementById("btn-host-load-cancel").addEventListener("click", () => {
    document.getElementById("host-load-modal").close();
  });
  document.getElementById("btn-host-load-confirm").addEventListener("click", async () => {
    const select = document.getElementById("host-file-select");
    ...
    try {
      ...
      const importedLevels = JSON.parse(selectedOption.dataset.levels);
      if (!Array.isArray(importedLevels)) {
        throw new Error("Invalid format: root object should be an array of levels.");
      }
      disableTestMode();
      levels = importedLevels;
      currentLevelIdx = 0;
      ...
      if (parts.length >= 2) {
        localStorage.setItem("hexGridAuthorName", parts[0]);
        defaultAuthorName = parts[0];
        defaultLevelName = parts.slice(1).join("_");
      }
      populateLevelSelect();
      loadLevel(0);
      document.getElementById("host-load-modal").close();
